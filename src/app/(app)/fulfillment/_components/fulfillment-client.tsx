...
const CHIPS = [
  { label: '待出貨', value: 'paid_only', desc: '已確認且已付款' },
  { label: '全部待出貨（含未付款）', value: 'all', desc: '已確認' },
];
...
export function FulfillmentClient({ orders, activeFilter }: Props) {
  ...
  const toggleFilter = useCallback(
    (value: string) => {
      ...
      if (value !== 'paid_only') params.set('filter', value);
  ...
  return (
    <>
      ...
      <div className="fixed bottom-16 left-0 right-0 lg:left-56 bg-white border-t shadow-lg px-4 py-3 flex gap-3 z-10">
        ...
        <Button
          variant="outline"
          onClick={() => router.push('/fulfillment/summary')}
          className="shrink-0"
        >
          出貨彙總表
        </Button>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={() => setDialogOpen(false)} disabled={isMarking}>
              略過
            </Button>
